#!/usr/bin/env bash
# Prepare a resumable run journal. Use --require-clean for $codex-tuner:run.
...
if [ -f "$JOURNAL" ]; then
  ...
  [ "$STORED_BRANCH" = "$BRANCH" ] || execute_task_die "run '$EXECUTE_TASK_RUN_ID' belongs to branch '$STORED_BRANCH', not '$BRANCH'"
  [ "$STORED_TARGET" = "$TARGET" ] || execute_task_die "run '$EXECUTE_TASK_RUN_ID' targets '$STORED_TARGET', not '$TARGET'"
  {
    echo
  ...
  mv "$META_TMP" "$META" || execute_task_die "cannot install run metadata"
  {
    ...
    [ -z "$DIRTY" ] && echo "- baseline worktree: clean" || echo "- baseline worktree: dirty"
    echo
    echo "## log"
