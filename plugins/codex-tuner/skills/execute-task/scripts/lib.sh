#!/usr/bin/env bash
# Shared state and validation helpers for execute-task scripts.
...
execute_task_init_root() {
  ...
  [ -n "$requested" ] || execute_task_die "run this skill inside a Git repository"
  ...
  cd "$EXECUTE_TASK_ROOT" 2>/dev/null \
    || execute_task_die "cannot enter repository root '$EXECUTE_TASK_ROOT'"
...
execute_task_validate_run_id() {
  ...
  case "$value" in
    *[!A-Za-z0-9._-]*)
      execute_task_die "run-id may contain only ASCII letters, digits, dot, underscore, and hyphen"
      ;;
  esac
...
execute_task_prepare_state() {
  local allow_tracked="${1:-}"
  local parent="$EXECUTE_TASK_ROOT/.agent-state"
  local ignore_file="$EXECUTE_TASK_STATE_DIR/.gitignore"
  local resolved tracked temporary path
  ...
  for path in "$parent" "$EXECUTE_TASK_STATE_DIR" "$EXECUTE_TASK_RUNS_DIR"; do
    ...
    [ ! -e "$path" ] || [ -d "$path" ] \
      || execute_task_die "state path is not a directory: $path"
  ...
  case "$resolved" in
    "$EXECUTE_TASK_ROOT"/*) ;;
    *) execute_task_die "state directory escapes repository: $resolved" ;;
  esac
  ...
  [ ! -e "$ignore_file" ] || [ -f "$ignore_file" ] \
    || execute_task_die "state ignore path is not a regular file"
...
execute_task_assert_regular_or_missing() {
  ...
  [ ! -e "$path" ] || [ -f "$path" ] \
    || execute_task_die "state path is not a regular file: $path"
}
