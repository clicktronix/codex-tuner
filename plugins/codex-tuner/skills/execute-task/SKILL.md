...
3. If `.codex/execute-task.md` exists, read it as repo-specific overrides. Otherwise infer commands
   and conventions from the repository. Ask only when an unresolved choice would materially change
   the result.
...
5. Choose a short run id and resolve `<skill-dir>` to the directory containing this `SKILL.md`.
   Start the local-only journal:

   ```bash
   bash "<skill-dir>/scripts/preflight.sh" <run-id> [target-branch]
   ```
...
- Run the repository's required full checks and CI. A task-introduced required check that remains
  red is a hard stop.
...
1. Run the artifact guard with the intended integration target when one exists:

   ```bash
   bash "<skill-dir>/scripts/guard-artifacts.sh" [target-ref]
   ```
...
5. Perform the action, verify the resulting remote/CI state, and journal the outcome.
...
Run journals live under `.codex/execute-task-runs/` and are excluded through `.git/info/exclude`.
To resume, read the journal, compare its branch/base SHA with the current repository, inspect new
changes since the last entry, and continue from the first unverified step. Never trust an old green
result after relevant code or dependencies changed.
